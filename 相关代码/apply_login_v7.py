...
if marker1 in content:
    content = content.replace(marker1, marker1 + user_cfg, 1)
...
if old_part in content:
    ...
    print("FAIL: main_login")

# 写回服务器（通过 base64 编码）
encoded = base64.b64encode(content.encode('utf-8')).decode('ascii')
stdin, stdout, stderr = client.exec_command(f'echo "{encoded}" | base64 -d > /opt/scis/streamlit_app.py')
