...
# 安装 python-docx
stdin, stdout, stderr = client.exec_command('/opt/scis/venv/bin/pip install python-docx')
print("安装输出:", stdout.read().decode('utf-8')[-500:])
print("安装错误:", stderr.read().decode('utf-8')[-500:])

# 验证安装
stdin, stdout, stderr = client.exec_command('/opt/scis/venv/bin/python -c "from docx import Document; print(\"OK\")"')
print("验证:", stdout.read().decode('utf-8').strip())
...
doc.add_page_break()
doc.add_heading('附件：前端UI优化与性能优化迭代记录（Phase 1-7）', level=1)

doc.add_heading('Phase 1：全局样式+导航栏升级', level=2)
p = doc.add_paragraph()
...
p.add_run('从父子选择器改为直接选择器 [data-testid="stSidebar"] button，解决Streamlit组件独立渲染问题')

doc.add_heading('Phase 2：首页Dashboard+数据可视化升级', level=2)
p = doc.add_paragraph()
...
doc.add_heading('Phase 3-5：各页面标题+内容区域升级', level=2)
p = doc.add_paragraph()
...
doc.add_heading('Phase 6：登录验证系统（权限控制）', level=2)
p = doc.add_paragraph()
...
doc.add_heading('Phase 7：性能测试与优化', level=2)
p = doc.add_paragraph()
p.add_run('数据缓存：').bold = True
p.add_run('st.cache_data(ttl=300) 包装所有数据加载函数，缓存时间5分钟，减少重复文件读取')
p = doc.add_paragraph()
...
with sftp.file('/tmp/append_docx2.py', 'w') as f:
    f.write(append_script.encode('utf-8'))

stdin, stdout, stderr = client.exec_command('/opt/scis/venv/bin/python /tmp/append_docx2.py')
